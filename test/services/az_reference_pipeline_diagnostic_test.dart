...
///   Stage 3 – _buildEntries() → CharacterReferenceEntry list
...
List<Character> _buildAZDataset() {
  const names = [
    'Aldric Vorn',    'Brenna Holt',   'Cassian Dread',  'Dyana Frost',
    'Elara Moon',     'Fynn Ashwood',  'Gavin Steele',   'Hira Vale',
    'Ivan Crest',     'Jora Blackwell','Kira Dusk',      'Lorn Ashveil',
    'Maren Coldwater','Nyra Emberveil','Oswin Thorn',    'Petra Vance',
    'Quinn Marsh',    'Ren Ashford',   'Sable Nightwood','Tavin Gale',
    'Urien Fenn',     'Vira Stoneway', 'Wren Coldmere',  'Xara Dunmore',
    'Yael Brightwood','Zara Embervale',
  ];
  return names.map((n) => Character(name: n, parentProjectId: _kProjectId)).toList();
}

String? _s1(String letter) {
  final t = '@$letter'; final tb = t.substring(0, t.length);
  final at = tb.lastIndexOf('@'); if (at == -1) return null;
...
List<CharacterReferenceEntry> _s3(List<Character> chars) {
  ...
  for (final c in chars) {
    ...
    entries.add(CharacterReferenceEntry(key: c.key, name: c.name, aliases: aliases));
  }
  return entries;
}

List<ReferenceCandidate> _s4(String q, List<CharacterReferenceEntry> e) =>
...
(bool, String, List<ReferenceCandidate>) _ctrl(String l, List<Character> cs) {
  ...
  final c = ReferenceAutocompleteController(quillController: q, charactersProvider: () => cs);
  q.replaceText(0, 0, '@$l', null);
  q.updateSelection(TextSelection.collapsed(offset: '@$l'.length), ChangeSource.local);
  c.onTextChanged();
...
void main() {
  ...
  group('Per-letter pipeline (A-Z)', () {
    for (final l in letters) {
      test('@$l full pipeline', () {
        ...
        expect(flt.any((c) => c.name == cn), isTrue, reason: 'S2 FAIL $cn not in provider');
        final ent = _s3(flt);
        final me = ent.where((e) => e.name == cn).toList();
        expect(me, isNotEmpty, reason: 'S3 FAIL $cn not in entries keys=${ent.map((e) => "${e.name}:${e.key}").toList()}');
        final cands = _s4(q!, ent);
        expect(cands.any((c) => c.displayName == cn), isTrue,
            reason: 'S4 FAIL resolve("$q") missed "$cn" entry.key=${me.first.key} cands=${cands.map((c) => c.displayName).toList()}');
        expect(true && cands.isNotEmpty, isTrue, reason: 'S5 FAIL overlay guard');
        ...
        expect(cc.any((c) => c.displayName == cn), isTrue,
            reason: 'Ctrl FAIL $cn not in ctrl.candidates: ${cc.map((c) => c.displayName).toList()}');
      });
    }
  });

  group('ReferenceEngine isolation A-Z', () {
    late List<CharacterReferenceEntry> entries;
    ...
    for (final l in letters) {
      test('resolve("$l") finds $l-character', () {
        ...
        expect(c.any((x) => x.displayName.startsWith(l)), isTrue,
            reason: 'Engine FAIL resolve("$l") → ${c.map((x) => x.displayName).toList()}');
      });
    }
  });
  ...
  group('Trigger guard edges', () {
    ...
    test('@A after space', () {
      const t = 'Hello @A'; final at = t.lastIndexOf('@');
      expect(t[at - 1], ' '); expect(t.substring(at + 1), 'A');
    });
    test('user@A blocked', () {
      const t = 'user@A'; final at = t.lastIndexOf('@'); final b = t[at - 1];
      expect(b != ' ' && b != '\n' && b != '\t', isTrue);
    });
  });
  ...
  test('A-Z SUMMARY TABLE', () {
    ...
    print('\n═══════════════════════════════════════════════════════════════════════════════════════');
    // ignore: avoid_print
    print('  A-Z REFERENCE PIPELINE DIAGNOSTIC │ Project $_kProjectId (26 chars)');
    // ignore: avoid_print
    print('═══════════════════════════════════════════════════════════════════════════════════════');
    // ignore: avoid_print
    print('  L │ Character           │ S1 │ S2 │ S3 │ key  │ S4  │ S5 │ Ctrl │ RESULT');
    // ignore: avoid_print
    print('  ──┼─────────────────────┼────┼────┼────┼──────┼─────┼────┼──────┼──────────────────');
    ...
    for (final l in letters) {
      ...
      if (!ok) { fails++; failLetters.add(l); if (fs != null) failStages.add(fs); }
      final res = ok ? 'PASS ✓' : 'FAIL ✗ ← $fs';
      // ignore: avoid_print
      print('  $l │ ${cn.padRight(19)} │ ${q ?? "X"} ${q != null ? " " : ""}│ ${inP ? "✓" : "✗"} │ ${inE ? "✓" : "✗"} │ $k │ ${s4ok ? "✓" : "✗"}(${c4.length}) │ ${s5ok ? "✓" : "✗"} │  ${ctOk ? "✓" : "✗"}  │ $res');
    }
    ...
    if (fails > 0) {
      ...
      if (failStages.length == 1) {
        // ignore: avoid_print
        print('  → ALL failures at SAME stage: ${failStages.first} – systemic.');
      }
    } else {
      // ignore: avoid_print
    ...
    // ignore: avoid_print
    print('       reference_engine.dart(S4) reference_autocomplete_overlay.dart(S5)');
    // ignore: avoid_print
