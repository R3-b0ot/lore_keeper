...
class _MapEditorCanvasState extends State<MapEditorCanvas> {
  ...
  void _handlePanStart(DragStartDetails details) {
    ...
    if (provider.currentTool == MapTool.select) {
      ...
      if (stampId != null) {
        ...
        if (activeStamp != null) {
          ...
          if (dist < 400) { // Clicked center (move)
            _activeDragType = 'move';
            _dragStartPos = localPosition;
            return;
          } else if (dx > 30 && dy > 30 && dx < 70 && dy < 70) { // Bottom right approx (scale)
            _activeDragType = 'scale';
            _dragStartPos = localPosition;
            _initialScale = activeStamp.scale;
            return;
          } else if (dx > -20 && dx < 20 && dy < -40 && dy > -80) { // Top center approx (rotate)
    ...
    if (provider.currentTool == MapTool.path || provider.currentTool == MapTool.brush) {
      setState(() {
  ...
  void _handlePanUpdate(DragUpdateDetails details) {
    ...
    if (provider.currentTool == MapTool.select && _activeDragType != 'none' && _dragStartPos != null) {
      final stampId = provider.selectedStampId;
      if (stampId != null) {
        MapStamp? activeStamp;
        for (var l in provider.activeMap!.layers) {
        ...
        if (activeStamp != null) {
          ...
          final newPos = Offset(activeStamp.x + delta.dx, activeStamp.y + delta.dy);
    ...
    if (_currentDrawingPath.length > 1) {
      if (provider.currentTool == MapTool.path) {
        provider.addPath(
          MapPath(
            id: const Uuid().v4(),
            controlPoints: _currentDrawingPath.map((o) => OffsetData.fromOffset(o)).toList(),
          ),
        );
        ...
        provider.addPolygon(
          MapPolygon(
            id: const Uuid().v4(),
            points: _currentDrawingPath.map((o) => OffsetData.fromOffset(o)).toList(),
    ...
    if (provider.currentTool == MapTool.select) {
      ...
      for (int i = provider.activeMap!.layers.length - 1; i >= 0; i--) {
        ...
        for (int j = layer.stamps.length - 1; j >= 0; j--) {
          ...
          if ((localPosition.dx - stamp.x).abs() < 50 && (localPosition.dy - stamp.y).abs() < 50) {
  ...
  Offset _getLocalPosition(Offset globalPosition) {
    ...
    final Offset transformedPosition = MatrixUtils.transformPoint(inverseMatrix, localPosition);
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: InteractiveViewer(
        ...
        scaleEnabled: provider.currentTool == MapTool.pan || provider.currentTool == MapTool.select,
        minScale: 0.1,
        maxScale: 5.0,
        constrained: false,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var layer in mapData.layers) {
      ...
      final polyPaint = Paint()
        ..color = const Color(0xFF6B8E23) // Olive drab land
      ...
      for (var mapPath in layer.paths) {
        ...
        final path = Path();
        path.moveTo(mapPath.controlPoints.first.dx, mapPath.controlPoints.first.dy);
        ...
        path.lineTo(mapPath.controlPoints.last.dx, mapPath.controlPoints.last.dy);
    ...
    if (currentDrawingPath.isNotEmpty) {
      final activePaint = Paint()
        ..color = currentTool == MapTool.brush ? Colors.green.withValues(alpha: 0.5) : Colors.blue
